...
def walkThroughDataset(dataSetDir):
    ...
    for scene_name in scene_list:
        ...
        for frame_data in frames_data:
            ...
            label_data = frame_data['labels'] if isinstance(
                frame_data['labels'], list) else [frame_data['labels']]
            ...
            for label in label_data:
                if len(label) != 0:
                    ...
                    if type == 'spline':
                        points = label['points']
                        points_new = get_lane_points(
                            spline=points, interval=0.02)

                        for point in points_new:
                            coord = (int(point[0]), int(point[1]))
                            cv2.circle(
                                img, coord, 4, subtype_colors[subtype], thickness=-1, lineType=8)
...
def get_lane_points(spline, interval=0.05):
    ...
    if degree == 2:
        ...
        raise NotImplementedError(
            f"Supports Bezier spline with degrees 2 or 3, but got {degree}")
